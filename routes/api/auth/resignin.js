        ...
        if (!email || !password) {
            ...
            if (!result[0]) {
                ...
                if (password !== db_password) {
                    ...
                    // 패스워드가 일치하는 경우 -> 새로운 비밀번호 변경 -> 로그인 성공
                } else {
                    salt = await cryptoPassword.salt();
                    new_password = await cryptoPassword.hashedPassword(new_password, salt);
                    let query = 'UPDATE users SET salt = ?, password = ? WHERE email = ?';
                    const result_user = await connection.query(query, [salt, new_password, (result[0].email)]);

                    let query2 = 'SELECT user_id, email, name, salt, password FROM users WHERE email = ?';
                    let result2 = await connection.query(query2, [email]);

                    await connection.commit();
                    let data = {
                        token: jwt.sign(result2[0])
                    };
