...
router.post('/', async(req, res) => {
    try {
        ...
        let { email, name, birth, phone, gender } = req.body;
        ...
        if (!email || !name || !birth || !phone || !gender || !password) {
            ...
            // 올바른 request를 보낸 경우
               ...
               /* // users에 회원 정보 데이터 삽입하는 쿼리
                ...
                query = 'INSERT INTO users '
                    + '(name, password, email, phone, birth, gender, salt) ' 
                    + 'VALUES (?, ?, ?, ?, ?, ?, ?)';
                const result_user = await connection.query(query, [name, password, email, phone, birth, gender, salt]);
